...
function Courses() {
    ...
    const fetchPurchasedCourses = async () => {
      try {
        ...
        if (token) {
          ...
          // console.log("Purchases Data",response);

          //sirf id  bas in CourseData
          setPurchasedCourses(
            response.data.courseData.map((course) => course._id)
          );
        }
      } catch (error) {
        console.log("error in fetchPurchasedCourses ", error);
      }
    };
  ...
  //tou pehle hum usestate banya ge then set the courseId using map
          ...
          {loading ? (
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
              {courses.map((course) => (
                <div
                  key={course._id}
                  className="h-auto border border-gray-200 rounded-lg p-4 shadow-sm py-6"
                >
                  <img
                    src={course.image.url}
                    alt={course.title}
                    className="rounded mb-4 h-[50%] w-[50%] mx-auto text-center items-center"
                  />
...
//changes so that same course not add twice
// const navigate = useNavigate();
...
//       const token = user?.token;
//       if (token) {
//         const response = await axios.get("http://localhost:1000/api/v1/user/purchases/", {
//           headers: {
//             Authorization: `Bearer ${token}`,
//           },
//           withCredentials: true,
...
//   to={`/buy/${course._id}`}
//   className="bg-orange-500 w-full text-white px-4 py-2 rounded-lg hover:bg-blue-900 duration-300"
//   onClick={(e) => handleBuyNow(course._id, e)}
// >
//   Buy Now
// </Link>

// // New code (using Button)
// <button
//   onClick={() => handleBuyNow(course._id)}
//   className="bg-orange-500 w-full text-white px-4 py-2 rounded-lg hover:bg-blue-900 duration-300"
// >
